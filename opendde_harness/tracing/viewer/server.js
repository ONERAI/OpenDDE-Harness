...
const STATE_DIR = process.env.TRACING_STATE_DIR || path.join(os.homedir(), '.opendde_harness', 'traces')
...
const PROTEIN_DESIGN_ROOT =
  process.env.OPENDDE_HARNESS_PROTEIN_DESIGN_ROOT || path.join(os.homedir(), '.opendde_harness', 'protein_design')
  ...
  for (const span of spans) {
    ...
    if (span?.sessionId) {
      ...
      if (span.workspaceDir)
        current.workspaceCounts.set(span.workspaceDir, (current.workspaceCounts.get(span.workspaceDir) || 0) + 1)
...
function projectSpanForDisplay(span, identityIndex) {
  ...
  const parentAliasSessionId =
    span.sessionId && !isUuidLike(span.sessionId) ? canonicalIdBySessionKey.get(span.sessionId) || null : null
...
function buildSpanSubtitle(name, attrs) {
  ...
  if (name === 'memory.store') {
    ...
    if (attrs['memory.deposit_summary'])
      return base ? `${base} → ${attrs['memory.deposit_summary']}` : attrs['memory.deposit_summary']
  ...
  if (name === 'memory.consolidate')
    return attrs['memory.message_count'] != null ? `${attrs['memory.message_count']} msgs` : ''
  if (name === 'memory.profile_refresh')
    return attrs['memory.sections_rewritten'] != null ? `${attrs['memory.sections_rewritten']} sections` : ''
...
function extractSessionsSpawnPayload(span, identityIndex) {
  const attrs = span?.attributes || {}
  if (
    String(attrs['tool.name'] || '')
      .trim()
      .toLowerCase() !== 'sessions_spawn'
  )
    return null
  ...
  const persistedText = persistedArtifact?.message?.details?.status
...
function buildTraceGroups(sessionSpans) {
  ...
  let rootCandidates = spans.filter(
    span =>
      (span.name === 'session.turn' || span.name === 'subagent.run') &&
      (!span.parentSpanId || !spanById.has(span.parentSpanId))
  )
  if (!rootCandidates.length) {
    rootCandidates = spans.filter(span => !span.parentSpanId || !spanById.has(span.parentSpanId))
  }

  const groups = rootCandidates.sort(compareSpansByTime).map(root => ({
    id: root.spanId,
    root,
  ...
  return keptGroups.map(group => {
    ...
    return {
      ...
      endTime:
        normalizedSpans.map(span => span.endTime).sort((a, b) => parseTime(b) - parseTime(a))[0] ||
        group.root?.endTime ||
        null,
...
function isSafeArtifactPath(filePath) {
  ...
  try {
    ...
    const root = fs.existsSync(ARTIFACTS_DIR) ? fs.realpathSync(ARTIFACTS_DIR) : path.resolve(ARTIFACTS_DIR)
...
function registeredProteinDesignTaskIds() {
  try {
    return new Set(
      fs
        .readdirSync(PROTEIN_DESIGN_ROOT, { withFileTypes: true })
        .filter(entry => entry.isDirectory())
        .filter(entry => {
          const taskDir = path.join(PROTEIN_DESIGN_ROOT, entry.name)
          return (
            fs.existsSync(path.join(taskDir, 'workflow.json')) || fs.existsSync(path.join(taskDir, 'snapshot.json'))
          )
        })
