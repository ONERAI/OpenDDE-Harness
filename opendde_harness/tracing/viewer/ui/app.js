const state = {
  ...
  descriptors: {},
  appView: 'trace',
  proteinDesignData: null,
  selectedProteinDesignRunId: null,
  proteinDesignSelectionPinned: false,
  proteinDesignLoading: false,
  proteinDesignRenderSignature: null,
...
const elements = {
  ...
  proteinDesignScene: document.getElementById('proteinDesignScene'),
...
const I18N = {
  en: {
    ...
    'view.api': 'API Calls',
    'view.trace': 'Traces',
    'view.proteinDesign': 'Protein design',
...
function captureOpenDetails() {
  ...
  elements.detailsBody?.querySelectorAll('details[data-detail-key][open]')?.forEach(node => {
...
function renderApiFilters() {
  ...
  elements.providerFilter.innerHTML = providers
    .map(
      value =>
        `<option value="${escapeHtml(value)}">${value === 'all' ? t('filter.allProvider') : escapeHtml(value)}</option>`
    )
    .join('')
  ...
  elements.modelFilter.innerHTML = models
    .map(
      value =>
        `<option value="${escapeHtml(value)}">${value === 'all' ? t('filter.allModel') : escapeHtml(value)}</option>`
    )
    .join('')
...
function aggregateApiOverview(calls) {
  ...
  summary.topModel = [...summary.providerModelCounts.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] || null
  return summary
}
...
function sortedTraces(session) {
  return [...(session?.traces || [])].sort((a, b) => new Date(b.startTime).getTime() - new Date(a.startTime).getTime())
}
...
function preferredSpan(trace) {
  if (!trace) return null
  return trace.spans.find(span => span.name === 'llm.call') || trace.spans[0] || null
}
...
function extractAvailableTools(systemPrompt) {
  if (!systemPrompt) return []
  const toolBlockMatch = systemPrompt.match(
...
function renderRequestMessage(m) {
  ...
  if (typeof c === 'string') body = c
    ...
    body = c
      .map(b =>
        b && typeof b === 'object' ? (b.type === 'text' ? b.text || '' : `[${b.type || 'block'}]`) : String(b)
      )
      .join('\n')
...
function renderModelOutputCard(span, artifacts) {
  ...
  // OpenClaw dialect: assistantTexts / lastAssistant. opendde dialect: output/content
  ...
  // Normalize opendde's flat {id,name,arguments} into the {function:{name,arguments}}
...
function renderSubagentCallCard(span, artifacts) {
  ...
  // opendde persists the subagent's final return ON the span itself — the
  ...
  // found nothing for opendde, and fell back to the empty-result placeholder
  ...
  const outputSource = childOutcome
    ...
    : resultText
      ? 'subagent result artifact'
      : 'spawn accepted payload'

  // Forward link: opendde runs the subagent as its OWN trace; this dispatch
...
function renderApiToolbar(calls) {
  const providers = [
    ...
    ...new Set(
      allApiCalls()
        .map(entry => entry.span.attributes?.['llm.provider'])
        .filter(Boolean)
    )
  ]
  const models = [
    ...
    ...new Set(
      allApiCalls()
        .map(entry => entry.span.attributes?.['llm.model'])
        .filter(Boolean)
    )
  ]
  const agents = [
    ...
    ...new Set(
      allApiCalls()
        .map(entry => entry.sessionAgentId)
        .filter(Boolean)
    )
  ]
          ...
          <label class="api-inline-field">
            ...
            <select id="apiAgentFilterInline">
              ${agents
                .map(
                  value =>
                    `<option value="${escapeHtml(value)}"${value === state.agent ? ' selected' : ''}>${value === 'all' ? t('filter.allAgent') : escapeHtml(value)}</option>`
                )
                .join('')}
            </select>
          </label>
          <label class="api-inline-field">
            ...
            <select id="apiProviderFilterInline">
              ${providers
                .map(
                  value =>
                    `<option value="${escapeHtml(value)}"${value === state.provider ? ' selected' : ''}>${value === 'all' ? t('filter.all') : escapeHtml(value)}</option>`
                )
                .join('')}
            </select>
          </label>
          <label class="api-inline-field">
            ...
            <select id="apiModelFilterInline">
              ${models
                .map(
                  value =>
                    `<option value="${escapeHtml(value)}"${value === state.model ? ' selected' : ''}>${value === 'all' ? t('filter.all') : escapeHtml(value)}</option>`
                )
                .join('')}
            </select>
          </label>
...
function renderMetadata(span) {
  ...
  const rows = [
    ...
    [
      'Subagent Kind',
      attrs['subagent.id'] ? 'named subagent' : span.name === 'subagent.call' ? 'derived subagent' : null
    ],
...
function renderSkillReadCard(trace, span, artifacts) {
  ...
  // opendde's read_file→skill.read carries content/params on the re-typed tool
  ...
  // opendde's so the card isn't empty.
...
function renderSessionTurnCard(trace, span) {
  ...
  const hasCaps =
    attrs['turn.tools'] !== undefined ||
    attrs['turn.skills'] !== undefined ||
    attrs['turn.plugin.backend'] !== undefined
  const overview = [
...
// back to top-level input/output (or an attribute dump). opendde's own rich cards
...
const CUSTOM_RENDERERS = {
  ...
  skillRead: (span, trace, artifacts) =>
    renderSkillReadCard(trace, span, artifacts) + renderSkillEvidenceCard(trace, span),
  ...
  if (span.input) {
    cards.push(
      memoryIoCard(
        'Input',
        span.input.kind || '',
        renderByKind(topLevelPayload(span.input, artifacts), span.input.kind || 'json')
      )
    )
  }
  if (span.output) {
    cards.push(
      memoryIoCard(
        'Output',
        span.output.kind || '',
        renderByKind(topLevelPayload(span.output, artifacts), span.output.kind || 'json')
      )
    )
  }
...
function renderContentLegacy(span, trace, artifacts) {
  ...
  if (span.name === 'skill.read') {
    ...
    // opendde's read_file→skill.read is a re-typed tool call: its Tool
  ...
  const artifactCards =
    span.name === 'session.turn'
      ...
      : (span.name === 'llm.call' ||
            span.name === 'tool.call' ||
            span.name === 'subagent.call' ||
            span.name === 'skill.read' ||
            span.name === 'memory.recall' ||
            span.name === 'memory.store' ||
            span.name === 'memory.feedback') &&
...
// Memory nodes as Input → Output, mirroring the llm/tool cards. Under the long-term memory
...
function renderMemoryStoreCard(span, artifacts) {
  ...
  if (status === 'pending') {
    outNote = 'async · ~30s'
    outText =
      "Not yet distilled — messages ingested as a raw memcell; the memory service's async cascade (~30s) hasn't produced episode/fact/foresight yet. Refresh later."
    ...
    const header = `The memory service distilled this from the turn's memcell${mc}\njoined by (session_id, timestamp), Δ≈${delta}ms. Profile (user.md) is a merged memory doc — async, not per-turn.`
    outText = `${header}\n\n${blocks || 'Ingested, but nothing was distilled from this turn.'}`
  } else {
    outText = 'Deposit not resolved (memory data unavailable to the viewer).'
  }
...
function renderMemoryFeedbackCard(span) {
  ...
  const outText =
    'No output — backend.feedback is a deliberate no-op in the memory backend: the skill-usage signal is captured but not consumed.'
  return (
    memoryIoCard('Feedback · Input', '', preBody(inText)) +
    memoryIoCard('Feedback · Output', 'memory no-op', preBody(outText))
  )
}
...
function render() {
  ...
  document.body.classList.toggle('app-view-protein-design', state.appView === 'protein-design')
  if (state.appView === 'api') {
  ...
  } else if (state.appView === 'protein-design') {
    renderProteinDesignWorkspace()
    restoreScrollState()
  } else {
...
function openProteinDesignTrace(run) {
  const session = (state.data?.sessions || []).find(item => item.sessionId === run.sessionId)
  if (!session) return
  const trace = (session.traces || []).find(item => item.traceId === run.traceId)
  if (!trace) return
  state.selectedSessionId = session.sessionId
  state.selectedTraceKey = trace.traceKey
  state.selectedSpanId = run.spanId || trace.tree?.[0]?.spanId || trace.spans?.[0]?.spanId || null
  state.appView = 'trace'
  state.detailsRenderSignature = null
  render()
}

function proteinDesignRenderSignature(payload, traceAvailable) {
  const run = payload?.run
  if (!run) return JSON.stringify([payload?.error || '', payload?.selectedRunId || '', payload?.runs?.length || 0])
  const lastEvent = run.events?.[run.events.length - 1] || {}
  const metricHeads = Object.entries(run.series || {}).map(([name, value]) => {
    const cycleBest = value?.cycleBest?.[value.cycleBest.length - 1]
    const globalBest = value?.globalBest?.[value.globalBest.length - 1]
    return [name, cycleBest?.cycle, cycleBest?.value, globalBest?.cycle, globalBest?.value]
  })
  return JSON.stringify([
    ...
    run.populationCandidateIds?.length || 0,
    run.tree?.nodes?.length || 0,
    run.events?.length || 0,
    lastEvent.event_id,
    lastEvent.status,
    lastEvent.timestamp,
    run.postFilter?.status,
    run.postFilter?.decisions?.length || 0,
    traceAvailable,
    metricHeads
  ])
}

function renderProteinDesignWorkspace() {
  const dashboard = window.ProteinDesignDashboard
  if (!dashboard || !elements.proteinDesignScene) return
  if (state.proteinDesignLoading && !state.proteinDesignData) {
    elements.proteinDesignScene.innerHTML = '<div class="protein-loading">Loading protein-design runs…</div>'
    return
  }
  const payload = state.proteinDesignData || { runs: [] }
  const run = payload.run
  const workspace = elements.proteinDesignScene.querySelector('#proteinCandidateWorkspace')
  if (
    workspace &&
    run &&
    workspace._proteinRunId === run.taskId &&
    (workspace.querySelector('details[open]:not(.protein-cycle-more)') ||
      (workspace.contains(document.activeElement) && document.activeElement.matches('input, select')))
  )
    return
  const traceAvailable = run
    ? (state.data?.sessions || []).some(
        session =>
          session.sessionId === run.sessionId && (session.traces || []).some(trace => trace.traceId === run.traceId)
      )
    : false
  const renderSignature = proteinDesignRenderSignature(payload, traceAvailable)
  if (
    state.proteinDesignRenderSignature === renderSignature &&
    elements.proteinDesignScene.querySelector('.protein-dashboard')
  )
    return
  state.proteinDesignRenderSignature = renderSignature
  dashboard.mount(elements.proteinDesignScene, run ? { ...payload, run: { ...run, traceAvailable } } : payload, {
    onRunChange: taskId => {
      state.selectedProteinDesignRunId = taskId
      state.proteinDesignSelectionPinned = true
      loadProteinDesignRun(taskId)
    },
    onOpenTrace: openProteinDesignTrace
  })
}

window.addEventListener('protein-design-ready', () => {
  if (state.appView === 'protein-design') renderProteinDesignWorkspace()
})

async function loadProteinDesignRun(taskId, runs = state.proteinDesignData?.runs || [], renderOnComplete = true) {
  if (!taskId || state.proteinDesignLoading) return
  state.proteinDesignLoading = true
  try {
    const response = await fetch(`/api/protein-design?run_id=${encodeURIComponent(taskId)}&ts=${Date.now()}`, {
      cache: 'no-store'
    })
    if (!response.ok) throw new Error(`Protein design API failed with status ${response.status}`)
    const detail = await response.json()
    state.proteinDesignData = {
      runs: detail.runs || runs,
      selectedRunId: detail.selectedRunId || taskId,
      run: detail.run || null
    }
    state.selectedProteinDesignRunId = state.proteinDesignData.selectedRunId
  } catch (error) {
    state.proteinDesignData = {
      runs,
    ...
    state.proteinDesignLoading = false
    if (renderOnComplete && state.appView === 'protein-design') render()
  }
}

async function loadProteinDesignData(renderOnComplete = true) {
  if (state.proteinDesignLoading) return
  state.proteinDesignLoading = true
  try {
    const response = await fetch(`/api/protein-design?ts=${Date.now()}`, { cache: 'no-store' })
    if (!response.ok) throw new Error(`Protein design API failed with status ${response.status}`)
    const summary = await response.json()
    const selected = window.ProteinDesignDashboard.selectRunId(
      summary.runs || [],
      summary.selectedRunId,
      state.selectedProteinDesignRunId,
      state.proteinDesignSelectionPinned
    )
    const selectionStillAvailable = (summary.runs || []).some(run => run.taskId === state.selectedProteinDesignRunId)
    if (!selectionStillAvailable) state.proteinDesignSelectionPinned = false
    state.proteinDesignData = {
      runs: summary.runs || [],
      selectedRunId: selected || null,
      run: null
    }
    state.selectedProteinDesignRunId = selected || null
    state.proteinDesignLoading = false
    if (selected) {
      await loadProteinDesignRun(selected, summary.runs || [], false)
      return
    }
  } catch (error) {
    state.proteinDesignData = { runs: [], selectedRunId: null, run: null, error: error.message }
  } finally {
    state.proteinDesignLoading = false
    if (renderOnComplete && state.appView === 'protein-design') render()
  }
...
async function loadData(options = {}) {
  ...
  try {
    ...
    if (state.appView === 'protein-design') await loadProteinDesignData(false)
...
elements.appViewButtons.forEach(button => {
  button.addEventListener('click', () => {
    ...
    render()
    if (state.appView === 'protein-design') loadProteinDesignData()
  })
})
...
function applyTheme(theme) {
  ...
  const backgroundColor = dark ? 0x191c24 : 0xffffff
  const workspace = document.getElementById('proteinCandidateWorkspace')
  workspace?._proteinViewer?.plugin?.canvas3d?.setProps({ renderer: { backgroundColor } })
  const preview = elements.proteinDesignScene?._treePreview?.viewer
  preview?.setBackgroundColor(backgroundColor)
  preview?.render()
  document.querySelectorAll('[data-structure-path]').forEach(stage => {
    stage._proteinViewer?.setBackgroundColor(backgroundColor)
    stage._proteinViewer?.render()
  })
}
...
if (!themeButton) {
  ...
  themeButton.className = 'ghost-button app-action'
  elements.refreshButton.before(themeButton)
}
let savedTheme = 'light'
try {
  savedTheme = localStorage.getItem('tracing:theme') || 'light'
} catch {}
...
themeButton.addEventListener('click', () => {
  ...
  applyTheme(theme)
  try {
    localStorage.setItem('tracing:theme', theme)
  } catch {}
})
