...
function writeFixture(stateDir, cycleArtifactPath) {
  ...
  fs.writeFileSync(path.join(logs, 'audit-spans.log'), `${JSON.stringify(run)}\n${JSON.stringify(cycle)}\n`, 'utf8')
...
test('serves projected OpenDDE Harness protein-design runs without changing the traces API', async t => {
  ...
  const selected = await (await fetch(`http://127.0.0.1:${port}/api/protein-design?run_id=task-1`)).json()
  assert.equal(selected.run.taskId, 'task-1')
  assert.deepEqual(selected.run.cdrRegionGroups, {
    D: [
      [1, 2],
      [4, 5],
      [8, 9]
...
test('does not follow a protein-design artifact symlink outside the tracing root', async t => {
  ...
  const projected = await (await fetch(`http://127.0.0.1:${port}/api/protein-design?run_id=task-1`)).json()
