...
PLUGIN_ID = "long-term-memory"
BACKEND_NAME = "longterm"
...
def configured_base_url(config: Any) -> str:
    ...
    value reports on a server nobody is using: someone who moved the server off
    port 18791 is told it is not running. Key order mirrors
    ``_plugin_stack._resolve_plugin_config_slice`` (plugin id, then contribution
    name) without building a registry, which the callers -- ``ddeharness doctor`` and
    the wizard -- have no other reason to do.
    """
    slices = getattr(getattr(config, "plugins", None), "config", None) or {}
    for key in (PLUGIN_ID, BACKEND_NAME):
...
def parse_capabilities(payload: Any) -> dict[str, bool]:
    ...
    its search lane: parsed two ways, ``ddeharness doctor`` and recall could disagree
...
def probe_capabilities(base_url: str = DEFAULT_MEMORY_BASE_URL) -> CapabilityReport:
