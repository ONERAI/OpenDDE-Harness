...
from opendde_harness.spine.events import RunnerEvent, TurnEnded, TurnEvent, TurnFailed, TurnStarted
...
# System origins share the system pool. SUBAGENT here is the result-reinjection
# turn; a subagent's own execution runs off the scheduler behind a separate gate.
_SYSTEM_ORIGINS = (Origin.SUBAGENT,)
...
class OriginPools:
    """Per-origin concurrency gates: a USER pool and a system pool, sized
    independently. No global cap (total concurrency is their sum) and no
    borrowing between them, so a user turn never waits on an LLM slot
    behind a system task.
    """
