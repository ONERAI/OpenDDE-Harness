"""Typer subcommand: `ddeharness tui` — launch the Ink+React TUI subprocess.
...
from opendde_harness.cli._log_file import (
...
# child, Node maps them to fixed numbers (3 / 4) via the OPENDDE_HARNESS_RPC_FD_*
...
# in OPENDDE_HARNESS_RPC_SOCKET and authenticates with the OPENDDE_HARNESS_RPC_TOKEN shared secret
...
_RPC_SOCKET_ENV: str = "OPENDDE_HARNESS_RPC_SOCKET"
_RPC_TOKEN_ENV: str = "OPENDDE_HARNESS_RPC_TOKEN"  # noqa: S105 -- env var name, not a secret
...
_TUI_PROGRESS_SINK = None


def _build_tui_agent_loop():
    """Construct the AgentLoop singleton served by ``turn.send``.

    The one place the harness wires a chat loop: config / provider / session
    manager / AgentLoop for config / provider / session manager / AgentLoop.
    ``run_turn`` lazily ``_start_executor`` + ``_connect_mcp``
    ...
    from opendde_harness.tui_rpc.errors import InternalError
    ...
    try:
        ...
        backend = maybe_build_memory_backend(config.workspace_path, config, registry=plugin_registry)
        plugin_tools = build_plugin_tools(
            ...
            progress_sink=_TUI_PROGRESS_SINK,
        )
        ...
        raise InternalError(
            ...
            data={
                ...
                "log_path": "~/.opendde_harness/logs/tui.log",
            },
        ) from e
...
async def _run_rpc_server_until_done(
    ...
    from opendde_harness.tui_rpc.methods import register_aligned_methods_except_system
    from opendde_harness.tui_rpc.methods.system import (
    ...
    # builds the loop that serves chat turns from a TUI subprocess. Eager build at server bring-up so a
    ...
    def _protein_design_progress(event) -> None:
        payload = (
            event.compact_payload()
            if hasattr(event, "compact_payload")
            else event.model_dump(mode="json")
            if hasattr(event, "model_dump")
            else event
        )
        asyncio.create_task(emitter.broadcast({"type": "protein_design.progress", "payload": payload}))

    global _TUI_PROGRESS_SINK
    _TUI_PROGRESS_SINK = _protein_design_progress
    try:
        agent_loop = _build_tui_agent_loop()
    except RpcError as e:
        build_error = e

    # Late-bind the QuestionBroker into the ask_user tool now that the loop (and
    # its tool registry) exists; the broker was built up-front.
    ...
    # config.* + session.* + terminal.* + slash routing + turn.*).
    # Keeping production aligned with the umbrella means any future
    # register_*_methods helper added in opendde_harness/tui_rpc/methods/__init__.py
    # is picked up automatically — no more registration drift where new
    # handlers worked in the demo runner but returned -32601 in `ddeharness tui`.
    ...
    try:
        ...
        # memory server, up to 30s) so it doesn't block first render.
...
def _spawn_with_rpc_socket(
    ...
    Topology (cross-platform: macOS / Linux / Windows):
        ...
        parent: socket()/bind()/listen() on 127.0.0.1:<ephemeral>; exports the
                ``host:port`` as ``OPENDDE_HARNESS_RPC_SOCKET`` and a random shared secret
                as ``OPENDDE_HARNESS_RPC_TOKEN``
        child:  reads ``OPENDDE_HARNESS_RPC_SOCKET``, ``net.createConnection({host,port})``,
                sends ``OPENDDE_HARNESS_RPC_TOKEN`` as the first line, then emits JSON-RPC
...
def run_subprocess_with_rpc(
    ...
    ``OPENDDE_HARNESS_RPC_SOCKET`` plus a random shared secret via ``OPENDDE_HARNESS_RPC_TOKEN``.
    ...
    # The child inherited the real terminal above; from here on this process
    # must not write to it (Ink owns the screen), so fds 1/2 go to the log.
    if log_path is None:
        return _run_rpc_session(proc, server_sock, auth_token, forward_signals)
    with redirect_terminal_fds_to_file(log_path):
        return _run_rpc_session(proc, server_sock, auth_token, forward_signals)


def _run_rpc_session(proc, server_sock, auth_token: str, forward_signals: bool) -> int:
    if forward_signals:
...
def _release_compute() -> None:
    """Ask the on-demand compute container to stop if idle; best-effort, never delays the exit."""
    try:
        from opendde_harness.plugin.protein_design.servers.local_service import release_if_idle

        release_if_idle(timeout=2.0)
    except Exception:
        pass
...
def tui(
    ...
    if (
        ...
        try:
            language = _config_language()
        except Exception:
            language = "en"
        try:
            node_runtime.install_node(translate=lambda en, zh: zh if language == "zh" else en)
        except node_runtime.NodeRuntimeError as exc:
    ...
    # `ddeharness upgrade`. The gateway reads that cache when it builds the session
    # info bundle.
    from opendde_harness.cli.update_notice import maybe_refresh_async
    ...
    # Only set it when --color was passed so a shell-level OPENDDE_HARNESS_TUI_COLOR
    # isn't clobbered by the "auto" default.
    if color is not None:
        os.environ["OPENDDE_HARNESS_TUI_COLOR"] = color

    # `--check` is a smoke test: tell the child to boot, prove stub init,
    # then exit 0 (no Ink render, no interactive TTY). The child reads
    # OPENDDE_HARNESS_TUI_CHECK from the environment it inherits from this process.
    ...
    if dev:
        ...
        # Derive npx from the validated node_path so OPENDDE_HARNESS_NODE's
        ...
        # OPENDDE_HARNESS_NODE at a standalone node binary with no sibling npx).
        derived_npx = Path(node_path).parent / "npx"
        if derived_npx.exists():
            npx = str(derived_npx)
        else:
            fallback = shutil.which("npx")
            if fallback is not None and os.environ.get("OPENDDE_HARNESS_NODE"):
                typer.echo(
                    f"⚠ OPENDDE_HARNESS_NODE was set but no `npx` next to {node_path};\n"
