...
writes its logs next to its own config rather than always to ``~/.opendde_harness``.

Env vars:
    OPENDDE_HARNESS_CLI_DEBUG=1  — additionally mirror DEBUG+ to stderr.
"""
...
from opendde_harness.config.paths import get_logs_dir
...
def _strip_tty_stream_handlers() -> None:
    ...
    libraries (e.g. the memory library's configure_logging) install after basicConfig runs.
    """
...
def redirect_terminal_fds_to_file(path: Path) -> Generator[None, None, None]:
    ...
    The memory library's embedded structlog prints directly to stdout via PrintLogger, bypassing
