...
three-namespace metadata lookup (``opendde_harness > nanobot > openclaw``).
...
Layers (highest priority first):
  ...
  builtin   : packaged ``memory_engine/skills/`` plus every plugin
                                            ``skills_dirs`` contribution
...
class SkillRegistry:
    ...
    def __init__(
        ...
        Args:
            ...
            plugin_skills_dirs: skill roots shipped by plugins, scanned as
                part of the ``builtin`` layer. Defaults to the activated
        ...
        self.plugin_skills_dirs = (
            list(plugin_skills_dirs) if plugin_skills_dirs is not None else active_registry().skills_dirs()
        )
        ...
        # (e.g. a memory-skill add) don't drop the whole cache.
    ...
    def invalidate_cache(self) -> None:
        ...
        when only one logical source changed (typical: memory skill
    ...
    def invalidate_source(self, source: str) -> None:
        ...
        on every memory evolver write so newly materialized
        ``memory`` SKILL.md files surface to the in-process
    ...
    def resolve_source_for_path(self, path: Path) -> str | None:
        ...
        layers.extend((path, "builtin") for path in self.plugin_skills_dirs)
    ...
    def _list_all_locked(self) -> list[SkillMeta]:
        ...
        # key, possibly numeric for memory skills); ``full_key`` is keyed
        # by (source, display name) so by-name lookups still work when
        # display name diverges from dir name (the memory service writes
        ...
        layers.extend((path, "builtin", True) for path in self.plugin_skills_dirs)
    ...
    def _build_meta(
        ...
        *,
        ...
        # ``stable_key`` is the directory name. For memory skills it is
        # the sqlite ``skills.id`` string (the pipeline materializes
        # ``<workspace>/skills/memory/<sqlite_id>/SKILL.md``); for
        # everything else it equals the display name (legacy convention).
        stable_key = skill_dir.name
        # Display name prefers frontmatter ``name`` so memory skills
...
def _parse_nested_metadata(raw: str) -> dict:
    """Extract OpenDDE Harness-namespaced metadata from the ``metadata`` JSON blob.

    Lookup order (first match wins): ``opendde_harness`` > ``nanobot`` > ``openclaw``.
    ...
    if not isinstance(data, dict):
        return {}
    for key in ("opendde_harness", "nanobot", "openclaw"):
