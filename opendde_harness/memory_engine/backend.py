...
- ``recall`` names the track explicitly: it takes ``user_id`` XOR
  ``agent_id`` (exactly one set). Dual-track backends (the bundled long-term one) route
...
- ``feedback`` is **allowed to be a no-op**. Most backends have no
  ...
  Protocol exposes the slot so dual-track backends can consume it
...
class MemoryBackend(Protocol):
    ...
    async def recall(
        ...
        *,
        ...
        prefixed opaque string. Dual-track backends route the
    ...
    async def feedback(self, signals: dict[str, Any]) -> None:
        ...
        Dual-track backends with confidence-based skills do useful
