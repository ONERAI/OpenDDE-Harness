...
The method and its notification payload are declared in ``openrpc.json``.
"""
...
async def question_respond(params: dict[str, Any], *, question_broker: "QuestionBroker") -> dict:
    ...
    ok = question_broker.cancel(key) if params.get("cancelled") else question_broker.reply(key, answer)
    return {"ok": ok}
